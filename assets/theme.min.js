    ...
    function() {
        ...
        theme.PriceRange = function() {
            ...
            return t.prototype = Object.assign({}, t.prototype, {
                ...
                error: function(e) {
                  var element = document.querySelector('.product-single__variants');
                  var elementValue = element.value;
                  var dataAttribute = element.options[element.selectedIndex].getAttribute('data-maxqty');
                  var qty__num_str,
                      qty__num = document.querySelector('.js-qty__num.id_'+elementValue);
                  if (qty__num != null){
                    qty__num_str = qty__num.value;
                  }
                  else {
                    qty__num_str = 0;
                  }
                  var minsQty = dataAttribute - qty__num_str;
                  var errorgetHtml = element.options[element.selectedIndex].getAttribute('data-name');
                  var errorHtml = 'Sorry, we currently only have '+minsQty+' of '+errorgetHtml+' item in stock - your shopping cart has been now updated.'
                  var htmlError = '';
                  if (e.description == "You can't add more "+errorgetHtml+" to the cart.") {
                    htmlError = errorHtml;
                  }else{
                    htmlError = e.description;
                  }
                    if (e.description) {
                        var t = this.form.querySelector(".errors");
                        t && t.remove();
                        var i = document.createElement("div");
                        i.classList.add("errors", "text-center"), i.textContent = htmlError, this.form.append(i), document.dispatchEvent(new CustomEvent("ajaxProduct:error", {
                            detail: {
...
//   Custom JS to prevent checkout price vailidity
    $(function() {
  $('body').on('click', '.cart__checkout_limit--ajax', function(evt) {
    var total = $(this).parents('.drawer__footer').find('.ajaxcart__item_subtotal .ajaxcart__item_subtotal_price').text();
    var selected_currency = $('.currency-flag').attr('data-flag');
    if( selected_currency =='GBP'){
      var min_limit = 100.00;
    }else if( selected_currency =='USD'){
      var min_limit = 128.00;
    }else if( selected_currency =='AUD'){
      var min_limit = 180.00;
    }else if( selected_currency =='CAD'){
      var min_limit = 172.00;
    }else if( selected_currency =='EUR'){
      var min_limit = 110.00;
    }else if( selected_currency =='PLN'){
      var min_limit = 502.00;
    }else{
      var min_limit = 100.00;
    }
    if( selected_currency =='EUR' || selected_currency =='PLN' ){
      var currency = total.replace(/[\0123456789,.0123456789 ]/g, '');
      var total = total.replace('.', '');
      var total = total.replace(',', '');
      var cart_total = parseFloat(total.replace(/[\$£€zl]/g, ''));
    ...
    // alert(cart_total);
    if( currency =='zl'){
      var min_ammount = min_limit + currency;
    }else{
      var min_ammount = currency + min_limit;
    }
    console.log(cart_total);
    if( cart_total >= min_limit ){
      if ($('#CartTermsDrawer').is(':checked')) {
      } else {
        alert(theme.strings.cartTermsConfirmation);
        $(this).removeClass('btn--loading');
      ...
      $(this).removeClass('btn--loading');
      $('.cart__checkout-wrapper .price_error').text('Minimum order limit is '+ min_ammount + ' excluding tax and shipping');
      return false;
    }
  });
  $(document).on('click', '.cart__checkout-wrapper .cart__checkout-min-ajax', function(evt) {
    var total = $(this).parents('.drawer__footer').find('.ajaxcart__item_subtotal .ajaxcart__item_subtotal_price').text();
    var selected_currency = $('.currency-flag').attr('data-flag');
    if( selected_currency =='GBP'){
      var min_limit = 100.00;
  ...
  $('body').on('click', '.cart__checkout_limit--page', function(evt) {
    var total = $(this).parents('.cart__page-col').find('.cart_item_subtotal .cart_item_subtotal_price').text();
    var selected_currency = $('.currency-flag').attr('data-flag');
    if( selected_currency =='GBP'){
      var min_limit = 100.00;
    }else if( selected_currency =='USD'){
    ...
    if( cart_total >= min_limit ){
      ...
      $(this).removeClass('btn--loading');
      $('.cart_price_error').text('Minimum order limit is '+ min_ammount + ' excluding tax and shipping');
      return false;
    }
  });
  $('body').on('click', '.cart__checkout--min', function(evt) {
    var total = $(this).parents('.cart__page-col').find('.cart_item_subtotal .cart_item_subtotal_price').text();
    var selected_currency = $('.currency-flag').attr('data-flag');
    if( selected_currency =='GBP'){
      var min_limit = 100.00;
  ...
  $('#eori-number').val('');
  $('.attr_eori_number_y [type="radio"]').on('change', function(){
    $('#eori-number').val('');
    var val= $(this).val();
    if(val == 'Yes'){
      $('.eori_number_span').show();
      $('[name="checkout"]').attr('disabled','disabled');

    }else{
      $('.eori_number_span').hide();
      $('[name="checkout"]').removeAttr('disabled','disabled');
    }
  });

  $('#eori-number').on('change', function(){
    console.log('gg');
    var invlaue = $(this).val();
    if(invlaue != ' '){
      $('[name="checkout"]').removeAttr('disabled','disabled');
    }else{
      $('[name="checkout"]').attr('disabled','disabled');
  ...
  function optionChange() {
    var option1 = $('[data-index="option1"]:checked').val();
    setTimeout(function(){ 
      $('input[data-index="option1"]').removeClass('disabled');
      $('input[data-index="option1"]').next('label').removeClass('disabled'); 
      $( '[name="id"] option' ).each(function( index ) {
...
//         console.log(title);
//         console.log(length);
        if (available === 'available' && data_title.indexOf(option1) >-1  ){
//         console.log(option1);
//         console.log(title);
//         console.log(length);
          if(length == 2){
            var title_1 = title[1].trim();
            //             console.log(title_1);
            $('input[value="'+ title_1+'"]').removeClass('disabled');
            $('input[value="'+ title_1+'"]').next('label').removeClass('disabled');
            //console.log(title[1]);
          }
          if(length > 2){
            var title_2 = title[2].trim();
            $('input[value="'+title[2]+'"]').removeClass('disabled');
            $('input[value="'+ title_1+'"]').next('label').removeClass('disabled');
            // console.log(title[2]);
          }
        }
  ...
  function variantChange() {
    var option1 = $('[data-index="option1"]:checked').val();
    var i=1;
    $( 'input[data-index="option2"]' ).each(function() {
      var option2 = jQuery(this).val();
      var findclass = jQuery(this).hasClass("disabled");
      if((!findclass) && (i==1)){
  ...
  jQuery("input[data-index='option1']").change(function(){ 
    setTimeout(function(){ 
      variantChange();
    },500);
  });

  $('[data-variant-input]').on('change', function(){
    optionChange();
  });


//   var productColorName = localStorage.getItem('productColorName');
//   console.log(productColorName);
//   setTimeout(function(){ 
//   $('.variant-input-wrap .variant-input').find('input[data-index="option1"]').val(productColorName);
//   $('.variant-input-wrap .variant-input').find('input[value="'+productColorName+'"]').trigger('click');
//     },1000);

});

$( document ).ready(function() {
  $('.collection-grid__wrapper .grid .grid-product__link').on('click', function(){
  var productColorName = $(this).attr('data-color');
    localStorage.setItem('productColorName', productColorName);
  });
  var productColorName = localStorage.getItem('productColorName');
  $('.collection-grid__wrapper .grid .grid-product__colors .color-swatch').on('click', function(){
  var productColorName = $(this).attr('data-color');
    localStorage.setItem('productColorName', productColorName);
  });
    setTimeout(function(){ 
 ...
 if(currentURL.indexOf('login') > -1 || currentURL.indexOf('account') > -1){
...
$(".custom-login").click(function(){
 var currenturl = localStorage.getItem('currenturl');
	console.log(currenturl);
  if(currenturl != null){
     $('[name="return_url"]').val(currenturl);
  }
